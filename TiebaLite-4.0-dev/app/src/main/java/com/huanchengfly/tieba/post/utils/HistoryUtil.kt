...
object HistoryUtil {
    ...
    private fun saveOrUpdate(history: History, async: Boolean = false) {
        ...
        val saveHistory = history.copy(count = 1, timestamp = Clock.System.now().toEpochMilliseconds())
        if (async) {
    ...
    private fun saveOrUpdateAsync(
        ...
        updateAsync(history) { success ->
            if (!success) {
                history.copy(count = 1, timestamp = Clock.System.now().toEpochMilliseconds())
                    .saveAsync()
                    .listen {
                        callback?.invoke(it)
                    }
