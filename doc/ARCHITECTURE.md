...
- `:core-domain:jvmTest` 通过，包含 Source 能力、Connector Registry 与默认 Feed 选择规则测试。
- `:core-data:jvmTest` 通过，包含刷新退避/认证失败策略与 Tieba 用户内容映射测试。
...
### G8：来源实例化、缓存一致性与论坛深度闭环

- 引入 `SourceDescriptor` / Source factory，将 Discourse 等来源从单例硬编码升级为可持久化、多实例、可启停的运行目录，同时保持 feature 零来源分支。
- 为频道、主题、评论和 Reader 文章定义统一的缓存新鲜度、过期失效与离线回退策略；刷新成功后精确失效分页，失败时保留可读缓存。
- 完成 Discourse 频道详情、图片、标签和作者映射，清除展示层硬编码；继续补齐 Tieba 图片回复、点赞与楼中楼等只有真实价值的能力。
- 将 NGA/AcFun 参考实现隔离到实验区；只有具备频道列表、主题列表、详情和错误处理 contract tests 后才能重新进入运行目录。
- 建立 Desktop 集成 smoke：来源切换、登录覆盖、发帖/回复路由、聚合分页、离线回退和数据库缓存一致性。
- 完成后统一执行一次 Desktop 验证并全量提交，继续避免按单一页面拆分小补丁。
