...
### G9：真实同步、Reader 自动化与发布门禁

- 删除当前打印日志即成功的 Local/WebDAV 假同步，实现版本化 `UserDataBundle`，覆盖来源描述符、Reader 订阅、收藏、阅读状态与设置，并定义导入校验、冲突合并和幂等语义。
- WebDAV 传输放在 common 数据层；只有文件选择器、系统目录和凭据存储等确有价值的边界才增加 Desktop/Android 平台实现。
- 让 Reader 的 `autoRefresh`、`refreshInterval` 与 `CachePolicy` 真正参与调度；补齐订阅编辑、OPML/JSON 导入导出、失败隔离和离线阅读闭环。
- 建立 Source conformance kit 与 Desktop 集成 smoke，覆盖动态来源恢复、登录覆盖、发帖/回复/点赞路由、分页刷新、离线旧缓存和数据库重启一致性。
- 清理剩余假实现与运行时 TODO，完成 Desktop package、数据库迁移、CI 固定工具链和可重复发布检查；本轮仍以一次集中验证、一次完整提交验收。
