...
### G17：发布候选交互、可访问性与性能门禁

- 为全局导航、侧栏、命令层、模态层和阅读器补齐键盘快捷键、焦点顺序、语义标签、触控目标与 reduced-motion 行为，覆盖 Compact 到 Large。
- 建立 Forum/Reader 真实长列表的滚动位置恢复、筛选切换、返回栈与详情预览状态测试；统一消除剩余 Material/Compose 废弃 API 和无效可空分支。
- 审计 SQL 查询索引、Paging loadSize/预取距离、图片加载尺寸和 Reader HTML 渲染边界，添加可重复的 Desktop 基准数据集与性能预算。
- 运行 Desktop 完整测试、编译、启动探针与 `createDistributable`，对发布镜像、图标、元数据和升级说明做最终候选检查；common 通过后再集中启动 Android 回归。
