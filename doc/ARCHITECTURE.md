...
### G10：Source 合约门禁与 Desktop 可发布产品

- 建立可复用 Source conformance kit：每个运行来源必须验证目录、详情/分页、能力声明与 Connector 注册一致，登录覆盖、回复/点赞等可选能力只能在 contract test 通过后暴露。
- 完成 Desktop 集成 smoke：真实组合根启动、数据库从 v1 迁移、动态来源重启恢复、断网旧缓存、同步备份恢复和 Reader 定时刷新形成可重复场景。
- 收敛仍有产品价值的 TODO：附件选择/上传、文章分享、历史来源名称、刷新错误分类；删除无调用价值的样例与占位路径。
- 建立固定 JDK/Gradle 的 CI 与 Desktop package 验证，补应用版本、发行说明、日志边界和可重复构建产物；完成后再评估 Android/iOS 平台投入。
