...
- 把 WorkspaceSession 从工作区级恢复扩展到 Forum 版块/筛选、Reader 来源/筛选、Feed 来源集合、稳定列表锚点和安全详情引用；只恢复仍存在的对象，草稿使用独立版本与显式丢弃路径。
- 将命令面板升级为领域命令注册表：来源级刷新、停用、重新登录、同步、导入导出和页面内命令按能力动态注册，并增加冲突、不可用原因和执行结果测试。
- 为来源健康补充连续失败、最后成功、缓存年龄、限流恢复时间和账号有效性；提供脱敏诊断包预览/复制，禁止导出 Cookie、密码、开发 API Key 与正文缓存。
- 加入大数据离线种子和查询结构预算，验证全局搜索、来源健康首屏、恢复启动与工作区切换；仍只运行 Desktop 门禁，不创建平台专属页面。
