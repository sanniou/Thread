...
- 把 Social Connector 纳入运行时来源注册、统一 Feed 和 Inbox ingestion，提供至少一个真实协议适配器的认证、双向分页、内容警告、媒体和互动闭环。
- 建立持久化内容关系图：引用、回复、转载、同作者、同标签与 canonical URL 去重形成可分页的“相关内容”和阅读路径，不让页面临时拼接关系。
- 完成高级智能集合编辑器、排序/分组/固定、首页小组件与恢复状态；规则执行从受控缓存扫描升级为索引化 SQL 查询和增量物化视图。
- 在 common 契约下补系统深链、分享、文件导入、通知权限/点击回跳和后台刷新桥接；当前仍只用 Desktop stub/fixture 验证共享业务，不复制 Android/iOS 页面。
- 扩大到 10,000 行混合内容、通知风暴、关系循环、Social cursor 失效和进程恢复测试，并完成 0.21.0 Desktop 发行镜像与 launcher smoke。
