...
- 建立全局命令面板，将工作区、来源、搜索、刷新、同步和诊断动作统一成可搜索命令；补快捷键发现页和冲突测试，触控入口仍保持完整。
- 建立来源健康中心：聚合登录有效性、最后成功刷新、连续失败、缓存年龄、限流等待和运行时实例状态，并提供来源级重试、停用、重新认证与导出诊断。
- 完成跨 Forum/Reader 的离线全局搜索和工作区会话恢复；恢复来源、筛选、列表位置、打开详情及草稿时必须使用版本化可迁移状态，不依赖平台存储实现。
- 收口 Desktop 发行诊断、升级说明、数据目录与安全导出；建立大数据种子下的搜索/启动/滚动结构门禁，继续只验证 Desktop，不提前分叉 Android/iOS 页面。
