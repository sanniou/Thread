...
### G20：统一活动中心、身份状态与扩展平台契约

- 建立跨 Forum/Reader/Feed 的活动中心，把刷新任务、发布草稿、同步作业、认证过期与来源安装状态组合成可筛选、可重试、可深链的统一队列。
- 将账号有效性从错误文本推断升级为显式来源身份状态，按 Connector 登录策略生成重新认证动作，并让页面动作、命令面板和 Operations 复用同一执行器及冲突锁。
- 把 Reader 导入导出、用户数据备份恢复、草稿继续/丢弃和来源管理纳入动态命令注册；动作声明可用条件、进行状态、危险等级和完成反馈。
- 为未来 Social/stream Connector 固定 timeline 游标、作者身份、互动能力、内容警告和媒体网格契约；UI 只按能力组合，不新增平台或来源硬编码分支。
- Desktop 继续作为唯一发布门禁，增加活动队列重启、并发动作冲突、深链失效回退与大规模混合时间线结构测试。
