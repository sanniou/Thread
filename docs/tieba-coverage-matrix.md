...
Thread 基线：`0.40.0`（P0/P1 已落地；已有主路径持续质量补齐）
...
| 搜索吧 | hybrid `searchForum` | API only | missing | P2（可并入全局搜索） |
| 搜索用户 | hybrid `searchUser` | API only | missing | P2 |
...
| 消息：回复/艾特/赞 | replyMe / atMe / agreeMe / msg | replyMe/atMe→Inbox；agreeMe 仍 API only | partial (0.40) | P2 补 agreeMe |
...
| Trend | — | — | ✅ |
| Membership / Store / Inbox | — | — | ✅ 非 Connector 服务（0.40） |

`SourceCapabilities` 当前 Tieba：`supportsSearch=true`、`supportsTopicCreation=true`（0.39）。
...
| **P2** | 0.41 | 签到/mSign、吧规、点踩/dislike、关注用户、搜索吧/用户、agreeMe、图片相册增强；继续主路径质量 |
| **P3** | 0.42+ | 资料编辑、屏蔽、历史云同步、设置 parity（**删帖/吧务 deferred**） |
