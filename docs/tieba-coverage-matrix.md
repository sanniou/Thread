...
Thread 基线：`0.51.0`（P0–P3a + 看帖/楼层回复 + 推荐不感兴趣 + 本地屏蔽扩列表 + 资料编辑 + 吧内搜帖 + 现代化 UI 主路径 + redesign 波次2；设置 parity 后续）
...
| 帖详情 + 楼层 | `pbPage` / Thread | `getTopicDetail` + `getTopicComments`（倒序 r=1 / 跳页 startPage） | done (0.43) | — |
| 楼中楼 | `pbFloor` / SubPosts | `SubCommentConnector` + sheet 分页 Load more | done (0.43 分页) | — |
| 只看楼主 / 倒序 | Thread 筛选 | `hasPoOnly` + `isReverse` + FilterBar | done (0.43 倒序) | — |
| 点赞 | `agree` / opAgree | `TiebaReactionConnector.upvote` + shared TBS | done (0.40 polish) | — |
| 点踩 / 不感兴趣 | `disagree` / `submitDislike` | downvote=`opAgree agree_type=5` + Topic DislikeButton；推荐流 `submitNotInterested` / submitDislike | done (0.42 downvote + 0.44 not-interested) | — |
...
| Login | ✅ | ✅ | ✅ |
| SubComments | — | — | ✅ |
| Reactions | — | ✅ | ✅ upvote + downvote + submitNotInterested（0.42/0.44） |
| Trend | — | — | ✅ |
...
| **P3b-g** | 0.52+ | 设置 parity、头像 multipart（**删帖/吧务 deferred**） |
