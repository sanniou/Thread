...
class NmbRepositoryImpl(
        ...
        return Pager(
            ...
            pagingSourceFactory = {
                QueryPagingSource(
                    ...
                    context = Dispatchers.Default,
                    countQuery = database.threadQueries.countHistoryThreads(),
    ...
    override suspend fun getSortedCookies(): List<Cookie> {
        return database.cookieQueries.getSortedCookies().asFlow().mapToList(Dispatchers.Default).first()
    }
    ...
    override suspend fun insertCookie(alias: String, cookie: String) {
        ...
        val count =
            database.cookieQueries.countCookies().asFlow().mapToList(Dispatchers.Default).first().size
        ...
        return Pager(
            ...
            pagingSourceFactory = {
                QueryPagingSource(
                    transacter = database.threadQueries,
                    context = Dispatchers.Default,
                    countQuery = database.threadQueries.countSearchThreads(query),
                    queryProvider = { limit, offset ->
        ...
        return Pager(
            ...
            pagingSourceFactory = {
                QueryPagingSource(
                    transacter = database.threadReplyQueries,
                    context = Dispatchers.Default,
                    countQuery = database.threadReplyQueries.countSearchReplies(query),
                    queryProvider = { limit, offset ->
        ...
        return Pager(
            ...
            pagingSourceFactory = {
                QueryPagingSource(
                    transacter = database.threadQueries,
                    context = Dispatchers.Default,
                    countQuery = database.threadQueries.countThreadsByUserHash(userHash),
                    queryProvider = { limit, offset ->
        ...
        return Pager(
            ...
            pagingSourceFactory = {
                QueryPagingSource(
                    transacter = database.threadReplyQueries,
                    context = Dispatchers.Default,
                    countQuery = database.threadReplyQueries.countRepliesByUserHash(userHash),
                    queryProvider = { limit, offset ->
