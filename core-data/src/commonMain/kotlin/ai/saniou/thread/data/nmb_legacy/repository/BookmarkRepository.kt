...
class BookmarkRepository(
    ...
    fun getBookmarks(): Flow<List<Bookmark>> {
        return db.bookmarkQueries.selectAll().asFlow().mapToList(Dispatchers.Default)
    }
    ...
    fun isBookmarked(postId: String): Flow<Boolean> {
        return db.bookmarkQueries.getById(postId).asFlow().mapToList(Dispatchers.Default).map { it.isNotEmpty() }
    }
}
