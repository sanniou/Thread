...
class NmbTrendSource(
    ...
    override fun getTrendPagingData(tab: TrendTab, params: TrendParams): Flow<PagingData<TrendItem>> {
        return Pager(
            ...
            pagingSourceFactory = {
                NmbTrendPagingSource(nmbSource, params)
            }
        ).flow
    }

    private class NmbTrendPagingSource(
        private val nmbSource: NmbSource,
        private val params: TrendParams
    ) : PagingSource<Int, TrendItem>() {
        override fun getRefreshKey(state: PagingState<Int, TrendItem>): Int? = null

        override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TrendItem> {
            ...
            return try {
                ...
                val targetDate = today.minus(this.params.dayOffset, DateTimeUnit.DAY)

                // 1. Try to get from local cache if not force refreshing
                if (!this.params.forceRefresh) {
                    if (this.params.dayOffset == 0) {
                        ...
                        if (localLatestReply != null) {
                            ...
                            if (replyDate == today) {
                                ...
                                return LoadResult.Page(parsedItems, null, null)
                            }
                        }
                    } else {
                ...
                LoadResult.Page(parsedItems, null, null)
